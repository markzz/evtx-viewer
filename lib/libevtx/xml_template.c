...
int _parse_template(evtx_xml_obj_t **obj, const char *bytes) {
    ...
    size = four_bytes_to_int32(bytes + 0x06);

    /* TODO: debug log next four bytes (next template def offset) */
    /* TODO: debug log template GUID */

    data_size = four_bytes_to_int32(bytes + 0x1E);

    pos += parse_fragment(obj, bytes + 0x22, 1);

    vspec_num_items = four_bytes_to_int32(bytes + pos);
    pos += 4;
    ...
    for (int i = 0; i < vspec_num_items; i++) {
        ...
        vspec[i][0] = two_bytes_to_int16(bytes + pos);
        vspec[i][1] = two_bytes_to_int16(bytes + pos + 2);
        pos += 4;
    }
    ...
    for (int i = 0; i < vspec_num_items; i++) {
        switch (vspec[i][1]) {
            ...
            case EVTX_TYPE_GUID:
                ...
                memcpy(values[i], bytes + pos, (size_t)vspec[i][0]);
                pos += vspec[i][0];
                break;
            case EVTX_TYPE_INT8:
            ...
            case EVTX_TYPE_REAL32: /* this one does MAGIC */
                ...
                *((int32_t*)values[i]) = (int32_t) four_bytes_to_int32(bytes + pos);
                pos += 4;
                break;
            ...
            case EVTX_TYPE_REAL64: /* this one does MAGIC */
                ...
                *((int64_t*)values[i]) = (int64_t) eight_bytes_to_int64(bytes + pos);
                pos += 8;
                break;
            ...
            case EVTX_TYPE_FILE_TIME:
                ...
                *((time_t*)values[i]) = _filetime_to_unix_time(eight_bytes_to_int64(bytes + pos));
                pos += 8;
                break;
